(() => {
  ...
  const PUBLIC_VERSION = "Versão Beta 1.0";
  ...
  function normalizeBrazilPhone(value) {
    ...
    // Celular brasileiro: DDD (2 dígitos) + número móvel de 9 dígitos iniciado por 9.
    if (!/^[1-9][0-9]9[0-9]{8}$/.test(national)) return "";
    return "+55" + national;
  }
  ...
  async function requestPasswordReset(event) {
    ...
    if (!phone) return setRecoveryMessage(messageId, "Informe um celular brasileiro válido com DDD, por exemplo (11) 99999-9999.");
    ...
    try {
      ...
      if (listing?.status === "available" && Number(listing.currentPrice) > 0) {
        ...
        if (listing.currency === "BRL") {
          ...
          renderSetupGoalLinkPreview("Preço usado como valor-alvo inicial. Você ainda pode ajustá-lo antes de criar a conta.", "available");
          updateSetupPreview();
        } else {
  ...
  async function createProfile() {
    ...
    if (!phoneE164) return setupError("Informe um celular brasileiro válido com DDD para recuperação por SMS.");
    ...
    if (goalLinkUrl && (!setupLinkedGoal || normalizeGoalLinkUrl(setupLinkedGoal.url) !== goalLinkUrl)) {
      return setupError("Clique em ‘Ler anúncio’ para validar o link e buscar o preço antes de criar a conta.");
    }
  ...
  async function refreshLinkedGoal() {
    ...
    if (Number.isFinite(lastCheck) && Date.now() - lastCheck < 30000) {
      showToast("Este anúncio acabou de ser consultado. Aguarde alguns segundos.");
      return;
    }
    ...
    try {
      ...
      if (updated?.status === "available" && Number.isFinite(newPrice)) {
        ...
        showToast(linkedListingStatusInfo(updated?.status).label);
      }
    } catch (error) {
      ...
      saveState();
      renderGoal();
      showToast("Não foi possível atualizar o anúncio.");
    }
  }
  ...
  function renderSettings() {
    ...
    $("#savePhoneBtn")?.addEventListener("click", async () => {
      ...
      if (!phoneE164) {
        box.textContent = "Informe um celular brasileiro válido com DDD, por exemplo (11) 99999-9999.";
