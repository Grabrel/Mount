(() => {
  ...
  const PUBLIC_VERSION = "Versão Beta 0.6";
  ...
  async function handleLogin(event) {
    ...
    try {
      let { data, error } = await cloudSignIn(username, password);

      // Migração automática de uma conta local v0.5.x.
      if (error) {
        ...
        if (localCredentialIsValid) {
          if (button) button.textContent = "Migrando conta…";
          try {
            await createCloudAccount(username, password, JSON.parse(JSON.stringify(state)));
            ({ data, error } = await cloudSignIn(username, password));
          } catch (migrationError) {
            if (migrationError.code === "username_taken") {
              setLoginError("Esse usuário já existe na Nervi Cloud. Confira a senha usada nessa conta.");
              return;
            }
