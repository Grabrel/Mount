(() => {
  ...
  const REMEMBER_DEVICE_KEY = "nervi_remember_device_v112";
  const APP_VERSION = "1.1.2";
  window.COFRINHO_VERSION = APP_VERSION;
  const PUBLIC_VERSION = "Versão Beta 1.1.2";
  ...
  async function handleSetupReadGoalLink() {
    ...
    try {
      ...
      if (listing?.status === "available" && Number(listing.currentPrice) > 0) {
        ...
        const targetInput = $("#setupGoalTarget");
        ...
        if (listing.currency === "BRL") {
          if (targetInput) targetInput.value = String(Number(listing.currentPrice).toFixed(2));
          renderSetupGoalLinkPreview("Preço usado como valor-alvo inicial. Você ainda pode ajustá-lo antes de enviar a solicitação.", "available");
          updateSetupPreview();
          showToast("Anúncio lido e valor preenchido com sucesso.", "success");
        } else {
          renderSetupGoalLinkPreview(`Preço identificado em ${listing.currency}. Como o planejamento do Nervi usa reais, o valor-alvo não foi alterado automaticamente.`, "available");
          showToast("Anúncio lido com sucesso.", "success");
        }
      } else if (listing?.status === "price_not_found") {
        renderSetupGoalLinkPreview("A página abriu, mas o Nervi não conseguiu identificar um preço principal. O link pode ser salvo mesmo assim.", "warning");
      } else {
  ...
  async function createProfile() {
    ...
    const goalLinkRaw = $("#setupGoalLink")?.value.trim() || "";
    const goalLinkUrl = normalizeGoalLinkUrl(goalLinkRaw);
    ...
    if (goalLinkRaw && !goalLinkUrl) return setupError("O link da meta não é válido.");
    if (goalLinkUrl && (!setupLinkedGoal || normalizeGoalLinkUrl(setupLinkedGoal.url) !== goalLinkUrl)) {
    ...
    try {
      ...
      state.goal = {
        ...
        endDate: cycleEnd,
        cycleNumber: 1,
        linkedListing: goalLinkUrl ? normalizeLinkedListing(setupLinkedGoal) : null,
        createdAt: nowISO()
      };
      state.goalCycles = [];
      state.movements = [];
