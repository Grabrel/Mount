(() => {
  ...
  const REMEMBER_DEVICE_KEY = "nervi_remember_device_v113";
  const APP_VERSION = "1.1.3";
  window.COFRINHO_VERSION = APP_VERSION;
  const PUBLIC_VERSION = "Versão Beta 1.1.3";
  ...
  const LOGIN_URL = `${SUPABASE_URL}/functions/v1/nervi-login`;
  const READ_GOAL_LINK_URL = `${SUPABASE_URL}/functions/v1/nervi-read-goal-link`;
  function rememberDevicePreference() {
    try { return localStorage.getItem(REMEMBER_DEVICE_KEY) === "1"; }
    catch { return false; }
  }

  function setRememberDevicePreference(enabled) {
    try { localStorage.setItem(REMEMBER_DEVICE_KEY, enabled ? "1" : "0"); } catch {}
  }

  const nerviAuthStorage = {
    getItem(key) {
      try {
        const primary = rememberDevicePreference() ? localStorage : sessionStorage;
    ...
    setItem(key, value) {
      try {
        const primary = rememberDevicePreference() ? localStorage : sessionStorage;
        const secondary = rememberDevicePreference() ? sessionStorage : localStorage;
  ...
  let setupLinkedGoal = null;
  let setupUsernameCheckTimer = null;
  let setupUsernameAvailability = { key: "", state: "idle" };
  ...
  function migrateState(parsed) {
    ...
    if (base.profile) {
      ...
      base.profile.onboardingSeen = base.profile.onboardingSeen === true;
      delete base.profile.phoneE164;
      delete base.profile.phone;
    }
    ...
    if (base.goal) {
      ...
      base.goal.linkedListing = normalizeLinkedListing(base.goal.linkedListing || base.goal.linkedGoal || null);
    }
    base.version = APP_VERSION;
    return base;
  }

  function normalizeLinkedListing(value) {
    if (!value || typeof value !== "object") return null;
    const url = String(value.url || "").trim();
    if (!url) return null;
    const history = Array.isArray(value.history)
      ? value.history
          .filter(entry => entry && Number.isFinite(Number(entry.price)) && entry.checkedAt)
          .map(entry => ({
            checkedAt: String(entry.checkedAt),
            price: Number(entry.price),
            currency: String(entry.currency || value.currency || "BRL").toUpperCase(),
            note: String(entry.note || "")
          }))
          .slice(-60)
      : [];
    const extras = Array.isArray(value.extraCosts)
      ? value.extraCosts
          .filter(item => item && item.label && Number.isFinite(Number(item.value)))
          .map(item => ({ label: String(item.label).slice(0, 60), value: Number(item.value), currency: String(item.currency || "BRL").toUpperCase() }))
          .slice(0, 8)
      : [];
    const currentPrice = Number(value.currentPrice ?? value.price);
    const initialPrice = Number(value.initialPrice ?? history[0]?.price ?? currentPrice);
    return {
      url,
      resolvedUrl: String(value.resolvedUrl || url),
      title: String(value.title || "Anúncio vinculado").slice(0, 220),
      image: String(value.image || "").slice(0, 1800),
      currency: String(value.currency || "BRL").toUpperCase().slice(0, 6),
      priceType: String(value.priceType || "purchase").slice(0, 40),
      currentPrice: Number.isFinite(currentPrice) && currentPrice > 0 ? currentPrice : null,
      initialPrice: Number.isFinite(initialPrice) && initialPrice > 0 ? initialPrice : null,
      extraCosts: extras,
      status: String(value.status || "unknown").slice(0, 40),
      checkedAt: value.checkedAt ? String(value.checkedAt) : "",
      lastSuccessAt: value.lastSuccessAt ? String(value.lastSuccessAt) : "",
  ...
  function saveState(successMessage = "Alteração salva com sucesso.") {
    state.version = APP_VERSION;
  ...
  async function createCloudAccount(username, password, statePayload) {
    let response;
    try {
      response = await fetch(CREATE_ACCOUNT_URL, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "apikey": SUPABASE_PUBLISHABLE_KEY
        },
        body: JSON.stringify({ username, password, state: statePayload })
      });
    } catch (networkError) {
      console.error("Nervi create account network:", networkError);
      const error = new Error("Não foi possível conectar ao serviço de contas da Nervi. Verifique sua internet e tente novamente.");
    ...
    if (!response.ok) {
      const error = new Error(payload.message || "Não foi possível enviar a solicitação da conta.");
  ...
  async function callPublicFunction(url, body) {
    ...
    try {
      ...
      console.error("Nervi Edge Function network:", networkError);
      const error = new Error("Não foi possível conectar aos serviços da Nervi. Verifique sua internet ou tente novamente em instantes.");
      error.code = "network_error";
      throw error;
    }
  ...
  async function cloudUsernameStatus(username) {
    const payload = await callPublicFunction(CHECK_USERNAME_URL, { username });
    return {
      exists: payload.exists === true,
      status: String(payload.status || (payload.exists ? "approved" : "missing")),
      loginLocked: payload.loginLocked === true,
      failedLoginAttempts: Number(payload.failedLoginAttempts || 0),
      remainingAttempts: Number.isFinite(Number(payload.remainingAttempts)) ? Number(payload.remainingAttempts) : 3
    };
  }

  async function cloudLogin(username, password) {
    return callPublicFunction(LOGIN_URL, { username, password });
  }

  function isSequentialPin(value) {
    const pin = String(value || "");
    if (!/^\d{6}$/.test(pin)) return false;
    return "0123456789".includes(pin) || "9876543210".includes(pin);
  }

  function validNewPin(value) {
    const pin = String(value || "");
    return /^\d{6}$/.test(pin) && !isSequentialPin(pin);
  }

  function setSetupUsernameStatus(stateName = "idle", message = "") {
    const input = $("#setupUsername");
    const status = $("#setupUsernameStatus");
    if (!input || !status) return;
    input.classList.remove("field-success", "field-error", "field-checking");
    status.className = "field-status";
    if (stateName === "available") {
      input.classList.add("field-success");
      status.classList.add("success");
    } else if (stateName === "taken" || stateName === "invalid" || stateName === "error") {
  ...
  async function checkSetupUsernameAvailability({ immediate = false } = {}) {
    clearTimeout(setupUsernameCheckTimer);
    const input = $("#setupUsername");
    const username = input?.value.trim() || "";
    const key = normalizeAccessUsername(username);
    setupUsernameAvailability = { key, state: "idle" };

    if (!username) {
      setSetupUsernameStatus("idle", "");
      return null;
    }
    if (!validAccessUsername(username)) {
      setSetupUsernameStatus("invalid", "Use de 3 a 30 caracteres: letras, números, ponto, hífen ou sublinhado.");
      setupUsernameAvailability = { key, state: "invalid" };
      return null;
    }

    const run = async () => {
      const currentKey = normalizeAccessUsername($("#setupUsername")?.value || "");
      if (currentKey !== key) return null;
      setSetupUsernameStatus("checking", "Verificando disponibilidade…");
      setupUsernameAvailability = { key, state: "checking" };
      try {
        const result = await cloudUsernameStatus(username);
        const stillCurrent = normalizeAccessUsername($("#setupUsername")?.value || "") === key;
        if (!stillCurrent) return result;
        if (result.exists) {
          setupUsernameAvailability = { key, state: "taken" };
          setSetupUsernameStatus("taken", "Este nome de usuário já está em uso.");
        } else {
          setupUsernameAvailability = { key, state: "available" };
          setSetupUsernameStatus("available", "✓ Nome de usuário disponível.");
        }
        return result;
      } catch (error) {
        if (normalizeAccessUsername($("#setupUsername")?.value || "") !== key) return null;
        setupUsernameAvailability = { key, state: "error" };
        setSetupUsernameStatus("error", "Não foi possível verificar o nome agora. Tente novamente.");
        return null;
      }
    };

    if (immediate) return run();
    setupUsernameCheckTimer = setTimeout(run, 450);
    return null;
  }

  function updateSetupPinMatchStatus() {
    const pin = $("#setupPassword")?.value || "";
    const confirm = $("#setupPasswordConfirm")?.value || "";
    const status = $("#setupPinMatchStatus");
    const confirmInput = $("#setupPasswordConfirm");
    if (!status || !confirmInput) return;
    ...
    if (pin === confirm && validNewPin(pin)) {
      confirmInput.classList.add("field-success");
      status.classList.add("success");
    ...
    } else if (!validNewPin(pin)) {
      confirmInput.classList.add("field-error");
      status.classList.add("error");
      status.textContent = "A senha ainda não atende às regras.";
    }
  }

  function setupGoalMode() {
    return $('input[name="setupGoalMode"]:checked')?.value === "link" ? "link" : "manual";
  }

  function setupEffectiveGoalTarget() {
    if (setupGoalMode() === "link") {
      const listing = normalizeLinkedListing(setupLinkedGoal);
      const price = Number(listing?.currentPrice);
      return listing?.status === "available" && listing?.currency === "BRL" && Number.isFinite(price) && price > 0 ? price : 0;
    }
    return Number($("#setupGoalTarget")?.value || 0);
  }

  function renderSetupGoalMode() {
    const mode = setupGoalMode();
    $("#setupGoalLinkMode")?.classList.toggle("hidden", mode !== "link");
    $("#setupGoalManualMode")?.classList.toggle("hidden", mode !== "manual");
    const targetCard = $("#setupLinkedTargetCard");
    const targetDisplay = $("#setupLinkedTargetDisplay");
    const target = setupEffectiveGoalTarget();
    if (targetCard) targetCard.classList.toggle("hidden", !(mode === "link" && target > 0));
    if (targetDisplay) targetDisplay.textContent = target > 0 ? money(target) : "—";
    updateSetupPreview();
  }

  function normalizeGoalLinkUrl(value) {
  ...
  async function readGoalLink(url) {
    const normalized = normalizeGoalLinkUrl(url);
    if (!normalized) throw new Error("Informe um link válido começando com http:// ou https://.");
    return callPublicFunction(READ_GOAL_LINK_URL, { url: normalized });
  }

  function goalPriceTypeLabel(value) {
    if (value === "monthly_rent") return "Aluguel mensal";
    if (value === "monthly_service") return "Serviço mensal";
    if (value === "sale") return "Compra / venda";
    return "Compra";
  }

  function linkedListingStatusInfo(status) {
    const map = {
      available: { icon: "🟢", label: "Disponível", className: "available" },
      price_not_found: { icon: "🟡", label: "Preço não identificado", className: "warning" },
      unavailable: { icon: "🔴", label: "Anúncio indisponível", className: "danger" },
      blocked: { icon: "🔒", label: "Consulta bloqueada pelo site", className: "warning" },
      unreachable: { icon: "⚠", label: "Não foi possível verificar", className: "warning" },
      unsupported: { icon: "⚠", label: "Página não suportada", className: "warning" }
    };
    return map[status] || { icon: "⚪", label: "Ainda não verificado", className: "neutral" };
  }
  ...
  function buildLinkedListing(url, payload, previous = null) {
    const checkedAt = payload.checkedAt || nowISO();
    const status = payload.status || "unreachable";
    const newPrice = Number(payload.price);
    const validNewPrice = Number.isFinite(newPrice) && newPrice > 0;
    const old = normalizeLinkedListing(previous);
    const oldPrice = Number(old?.currentPrice);
    const currency = String(payload.currency || old?.currency || "BRL").toUpperCase();
    const history = Array.isArray(old?.history) ? [...old.history] : [];

    if (status === "available" && validNewPrice) {
      const changed = !Number.isFinite(oldPrice) || Math.abs(oldPrice - newPrice) >= 0.005;
      if (changed) {
        history.push({
          checkedAt,
          price: newPrice,
          currency,
          note: old ? "Preço alterado" : "Valor inicial"
        });
      }
    }

    return normalizeLinkedListing({
      url: normalizeGoalLinkUrl(url) || old?.url || url,
      resolvedUrl: payload.resolvedUrl || old?.resolvedUrl || url,
      title: payload.title || old?.title || "Anúncio vinculado",
      image: payload.image || old?.image || "",
      currency,
      priceType: payload.priceType || old?.priceType || "purchase",
      currentPrice: status === "available" && validNewPrice ? newPrice : old?.currentPrice,
      initialPrice: old?.initialPrice || (status === "available" && validNewPrice ? newPrice : null),
      extraCosts: Array.isArray(payload.extraCosts) ? payload.extraCosts : (old?.extraCosts || []),
      status,
      checkedAt,
      lastSuccessAt: status === "available" ? checkedAt : (old?.lastSuccessAt || ""),
      lastError: status === "available" ? "" : String(payload.message || payload.detail || "Não foi possível atualizar o anúncio."),
      history: history.slice(-60)
    });
  }

  function linkedListingDelta(listing) {
    const item = normalizeLinkedListing(listing);
    if (!item || !Number.isFinite(Number(item.currentPrice)) || !Number.isFinite(Number(item.initialPrice))) return null;
    return Number(item.currentPrice) - Number(item.initialPrice);
  }

  function linkedPrice(value, currency = "BRL") {
    const number = Number(value);
    if (!Number.isFinite(number)) return "—";
    try {
      return new Intl.NumberFormat("pt-BR", { style: "currency", currency: String(currency || "BRL").toUpperCase() }).format(number);
    } catch {
      return money(number);
    }
  }

  function linkedListingExtrasHtml(listing) {
    const extras = normalizeLinkedListing(listing)?.extraCosts || [];
    if (!extras.length) return "";
    return `<div class="linked-goal-extras">${extras.map(item => `<span>${escapeHtml(item.label)} <strong>${linkedPrice(item.value, item.currency)}</strong></span>`).join("")}</div>`;
  }

  function hideAllScreens() {
    ["loginScreen", "recoveryScreen", "aboutScreen", "welcomeScreen", "setupScreen", "mainScreen"].forEach(id => {
  ...
  function showAfterLogin() {
    if (state.profile?.onboardingSeen === true) showMain();
    else showWelcome();
  }


  function setLoginError(message = "", kind = "error") {
    const box = $("#loginError");
    if (!box) return;
    box.textContent = message;
    box.classList.toggle("hidden", !message);
    box.classList.toggle("success", Boolean(message) && kind === "success");
    box.classList.toggle("pending", Boolean(message) && kind === "pending");
  }
  ...
  function showLogin(prefill = true) {
    ...
    if ($("#rememberDevice")) $("#rememberDevice").checked = rememberDevicePreference();
    syncThemeButtons();
  ...
  async function handleLogin(event) {
    ...
    const rememberDevice = $("#rememberDevice")?.checked === true;
    ...
    try {
      const payload = await cloudLogin(username, password);
      if (!payload?.session?.access_token || !payload?.session?.refresh_token) {
        throw new Error("A sessão da Nervi não foi recebida corretamente.");
      }
      ...
      if (err?.code === "username_incorrect") {
        setLoginError("Usuário incorreto.");
      } else if (err?.code === "account_pending") {
        setLoginError("Sua conta ainda aguarda aprovação do gerente da Nervi.", "pending");
      } else if (err?.code === "account_blocked") {
        setLoginError("O acesso desta conta está bloqueado pelo gerente da Nervi.");
      } else if (err?.code === "login_locked" || payload.loginLocked === true) {
        setLoginError("Conta bloqueada após 3 tentativas incorretas. Somente o gerente da Nervi pode desbloquear o acesso.");
      } else if (err?.code === "wrong_password") {
        const remaining = Math.max(0, Number(payload.remainingAttempts ?? 0));
        if (remaining <= 0) {
          setLoginError("Senha incorreta. As 3 tentativas foram utilizadas e a conta foi bloqueada. Entre em contato com o gerente da Nervi.");
        } else {
          setLoginError(`Senha incorreta. ${remaining === 1 ? "Resta 1 tentativa" : `Restam ${remaining} tentativas`} antes do bloqueio.`);
        }
  ...
  function resetSetupForNewAccount() {
    ...
    const manualMode = $('input[name="setupGoalMode"][value="manual"]');
    if (manualMode) manualMode.checked = true;
    setSetupUsernameStatus("idle", "");
    updateSetupPinMatchStatus();
    renderSetupGoalLinkPreview();
    renderSetupGoalMode();
    addSetupPlannedRow();
  }
  ...
  function applyTheme(theme, persist = true) {
    ...
    syncThemeButtons();
    if (persist) showToast(`Aparência alterada para modo ${normalized === "dark" ? "escuro" : "claro"}.`, "success");
  }
  ...
  function showSetup() {
    ...
    renderSetupGoalMode();
    updateSetupPinMatchStatus();
    checkSetupUsernameAvailability();
    updateSetupPreview();
  }
  ...
  function renderSetupPlanned() {
    ...
    if (!setupPlanned.length) list.innerHTML = `<div class="empty-state">Nenhum gasto previsto. Você pode enviar a solicitação assim mesmo.</div>`;
  }
  function renderSetupGoalLinkPreview(message = "", kind = "") {
    const box = $("#setupGoalLinkPreview");
    if (!box) return;
    const listing = normalizeLinkedListing(setupLinkedGoal);
    if (!listing && !message) {
      box.className = "goal-link-preview hidden";
      box.innerHTML = "";
      return;
    }
    if (!listing) {
      box.className = `goal-link-preview ${kind || "warning"}`;
      box.textContent = message;
      return;
    }
    const status = linkedListingStatusInfo(listing.status);
    const price = Number(listing.currentPrice);
    const delta = linkedListingDelta(listing);
    box.className = `goal-link-preview ${status.className}`;
    box.innerHTML = `
      <div class="goal-link-preview-head">
        <span>${status.icon} ${escapeHtml(status.label)}</span>
        <small>${escapeHtml(goalPriceTypeLabel(listing.priceType))}</small>
      </div>
      <strong>${escapeHtml(listing.title)}</strong>
      ${Number.isFinite(price) && price > 0 ? `<div class="goal-link-preview-price">${linkedPrice(price, listing.currency)}</div>` : ""}
      ${delta !== null && Math.abs(delta) >= .005 ? `<small>${delta < 0 ? "↓" : "↑"} ${linkedPrice(Math.abs(delta), listing.currency)} desde o primeiro valor</small>` : ""}
      ${linkedListingExtrasHtml(listing)}
      ${message ? `<small class="goal-link-note">${escapeHtml(message)}</small>` : ""}
    `;
  }

  async function handleSetupReadGoalLink() {
    const input = $("#setupGoalLink");
    const button = $("#setupReadGoalLink");
    const url = normalizeGoalLinkUrl(input?.value || "");
    if (!url) {
      setupLinkedGoal = null;
      renderSetupGoalLinkPreview("Informe um link válido começando com http:// ou https://.", "warning");
      return;
    }
    if (button) {
      button.disabled = true;
    ...
    try {
      const payload = await readGoalLink(url);
      setupLinkedGoal = buildLinkedListing(url, payload, null);
      const listing = normalizeLinkedListing(setupLinkedGoal);
      if (listing?.status === "available" && Number(listing.currentPrice) > 0) {
        const nameInput = $("#setupGoalName");
        if (nameInput && (!nameInput.value.trim() || nameInput.value.trim() === "Minha reserva")) {
          nameInput.value = listing.title.slice(0, 80);
        }
        const category = $("#setupGoalCategory");
        if (category) category.value = listing.priceType === "monthly_rent" ? "Moradia" : "Compra planejada";
        if (listing.currency === "BRL") {
          renderSetupGoalLinkPreview("Preço definido automaticamente como valor da meta. Para escolher outro valor, altere o modo para ‘Informar valor manualmente’.", "available");
          renderSetupGoalMode();
          showToast("Anúncio lido e valor da meta definido com sucesso.", "success");
        } else {
          renderSetupGoalLinkPreview(`Preço identificado em ${listing.currency}. O modo por link exige um preço em reais; use o modo manual para definir uma meta em BRL.`, "warning");
        renderSetupGoalMode();
          showToast("Anúncio lido com sucesso.", "success");
        }
      } else if (listing?.status === "price_not_found") {
        renderSetupGoalLinkPreview("A página abriu, mas o Nervi não conseguiu identificar um preço principal. Para usar este anúncio como valor da meta, é necessário identificar um preço; você também pode mudar para o modo manual.", "warning");
        renderSetupGoalMode();
      } else {
        renderSetupGoalLinkPreview(listing?.lastError || "Não foi possível validar este anúncio agora.", linkedListingStatusInfo(listing?.status).className);
      }
    } catch (error) {
      setupLinkedGoal = null;
      renderSetupGoalLinkPreview(error?.message || "Não foi possível ler o anúncio agora.", "warning");
      renderSetupGoalMode();
    } finally {
      renderSetupGoalMode();
      if (button) {
        button.disabled = false;
        button.textContent = "🔗 Ler anúncio";
      }
    }
  }
  ...
  function updateSetupPreview() {
    ...
    if (detail) {
      detail.textContent = target > 0
        ...
        : (setupGoalMode() === "link" ? "Leia um anúncio com preço em reais para calcular." : "Informe o valor-alvo para calcular.");
    }
  ...
  function setupError(message = "") {
    ...
    box.textContent = cleanDialogText(message); box.classList.remove("hidden");
  }
  async function createProfile() {
    ...
    const goalMode = setupGoalMode();
    const goalLinkRaw = goalMode === "link" ? ($("#setupGoalLink")?.value.trim() || "") : "";
    const goalLinkUrl = goalMode === "link" ? normalizeGoalLinkUrl(goalLinkRaw) : "";
    const goalTarget = setupEffectiveGoalTarget();
    ...
    const usernameKey = normalizeAccessUsername(username);
    if (setupUsernameAvailability.key !== usernameKey || setupUsernameAvailability.state !== "available") {
      const checked = await checkSetupUsernameAvailability({ immediate: true });
      if (!checked) return setupError("Não foi possível confirmar a disponibilidade do nome de usuário agora.");
      if (checked.exists) return setupError("Esse nome de usuário já está em uso. Escolha outro.");
    }
    if (!/^\d{6}$/.test(password)) return setupError("A senha deve ter exatamente 6 números.");
    if (isSequentialPin(password)) return setupError("Escolha 6 números que não estejam em sequência, como 123456 ou 654321.");
    if (!validNewPin(password)) return setupError("Escolha uma senha válida de 6 números.");
    ...
    if (goalMode === "link") {
      if (!goalLinkRaw || !goalLinkUrl) return setupError("Informe um link válido para usar o modo por anúncio.");
      if (!setupLinkedGoal || normalizeGoalLinkUrl(setupLinkedGoal.url) !== goalLinkUrl) {
        return setupError("Clique em ‘Ler anúncio’ para validar o link e buscar o preço antes de enviar a solicitação.");
      }
      const linked = normalizeLinkedListing(setupLinkedGoal);
      if (linked?.status !== "available" || linked?.currency !== "BRL" || !(Number(linked.currentPrice) > 0)) {
        return setupError("O Nervi precisa identificar um preço válido em reais para criar a meta por link. Você pode tentar novamente ou usar o modo manual.");
      }
    }
    if (!(goalTarget > 0)) return setupError(goalMode === "link" ? "Leia o anúncio para definir o valor da meta." : "O valor-alvo precisa ser maior que zero.");
    ...
    try {
      ...
      state.goal = {
        ...
        endDate: cycleEnd,
        cycleNumber: 1,
        linkedListing: goalMode === "link" && goalLinkUrl ? normalizeLinkedListing(setupLinkedGoal) : null,
        createdAt: nowISO()
      };
      state.goalCycles = [];
      state.movements = [];
      ...
      await createCloudAccount(username, password, accountState);

      // O perfil financeiro só é promovido para a conta oficial depois que o
      // gerente altera o status para "approved" no Supabase.
      replaceState(previousState);
      saveStateLocalOnly();
      showLogin(false);
      if ($("#loginUsername")) $("#loginUsername").value = username;
      setLoginError("✓ Solicitação enviada com sucesso. Aguarde a aprovação do gerente da Nervi para entrar.", "success");
    } catch (err) {
      ...
      if (err?.code === "username_taken" || err?.status === 409) {
        ...
        setupError(err?.message || "Não foi possível enviar a solicitação da conta agora.");
      }
    } finally {
      suppressPersistence = false;
      if (button) {
        button.disabled = false;
        button.textContent = "Enviar solicitação de conta";
      }
    }
  }
  ...
  function renderDashboard() {
    ...
    const freeClass = plan.freeAfterCommitments < 0 ? "danger" : "good";
    ...
    $("#content").innerHTML =
      contentWindow(`INÍCIO — ${escapeHtml(monthLabel(selectedMonth))}`, `
        <section class="dashboard-balance-hero ${freeClass}" aria-label="Saldo livre">
          <div>
            <span class="dashboard-balance-label">Livre de verdade</span>
            <strong>${money(plan.freeAfterCommitments)}</strong>
            <p>O que sobra depois dos compromissos que ainda faltam pagar.</p>
          </div>
          <div class="dashboard-balance-context">
            <div><span>Disponível hoje</span><strong>${money(summary.available)}</strong></div>
            <div><span>Ainda comprometido</span><strong>${money(plan.remainingCommitments)}</strong></div>
          </div>
        </section>

        <div class="metric-grid dashboard-metrics-secondary">
          ${metric("Renda recebida", summary.income, "secondary-metric")}
          ${metric("Gastos realizados", summary.expenses, "secondary-metric")}
          ${metric("Reservado p/ meta", summary.goal, "secondary-metric")}
          ${metric("Livre previsto do mês", plan.projectedFree, `secondary-metric ${plan.projectedFree < 0 ? "danger" : ""}`)}
        </div>

        <div class="dashboard-tertiary-row">
          <span><strong>${money(plan.plannedTotal)}</strong> em gastos previstos</span>
          <span><strong>${money(plan.remainingCommitments)}</strong> ainda pendentes</span>
        </div>`)
  ...
  function openQuickExpenseDialog() {
    if (!state.profile || !state.goal) return;
    const defaultCategory = EXPENSE_CATEGORIES[0];
    openDynamicDialog({
      title: "＋ REGISTRAR GASTO",
      body: `
        <div class="quick-expense-summary">
          <span>Mês</span><strong>${escapeHtml(monthLabel(selectedMonth))}</strong>
          <span>Livre antes do gasto</span><strong>${money(planningSummary(selectedMonth).freeAfterCommitments)}</strong>
        </div>
        <div class="dialog-form-grid quick-expense-grid">
          <label>Categoria<select id="quickExpenseCategory">${EXPENSE_CATEGORIES.map(category => `<option>${escapeHtml(category)}</option>`).join("")}</select></label>
          <label>Prioridade<select id="quickExpensePriority">${[1,2,3].map(level => `<option value="${level}">${PRIORITIES[level]}</option>`).join("")}</select></label>
          <label>Descrição<input id="quickExpenseDescription" maxlength="120" placeholder="O que foi pago?"></label>
          <label>Valor<input id="quickExpenseValue" type="number" min="0.01" step="0.01" inputmode="decimal" placeholder="0,00"></label>
          <label>Data real<input id="quickExpenseDate" type="date" value="${defaultDateForMonth(selectedMonth)}"></label>
        </div>
        <div id="dynamicFormError" class="message error hidden"></div>
        <div class="dialog-actions">
      ...
      onOpen: () => {
        const category = $("#quickExpenseCategory");
        const priority = $("#quickExpensePriority");
        const syncPriority = () => {
          priority.value = String(state.categoryPriorities[category.value] || DEFAULT_PRIORITIES[category.value] || 3);
        };
        syncPriority();
        category.addEventListener("change", syncPriority);
        setTimeout(() => $("#quickExpenseDescription")?.focus(), 0);
      },
      onSubmit: async () => {
        try {
          const category = $("#quickExpenseCategory").value;
          const priority = Number($("#quickExpensePriority").value);
          const description = $("#quickExpenseDescription").value.trim();
          const value = Number($("#quickExpenseValue").value);
          const paymentDate = $("#quickExpenseDate").value;
          if (!description) throw new Error("Informe a descrição do gasto.");
          if (!(value > 0)) throw new Error("Informe um valor maior que zero.");
          if (!paymentDate) throw new Error("Informe a data real do gasto.");
          await addMovement({ type: "GASTO", category, description, value, priority, referenceMonth: selectedMonth, paymentDate });
          closeDynamicDialog();
          navigate(currentView);
        } catch (err) {
          showDynamicError(err?.message || "Não foi possível registrar o gasto.");
        }
      }
    });
  }
  ...
  function linkedGoalDashboardHtml() {
    const listing = normalizeLinkedListing(state.goal?.linkedListing);
    if (!listing) return "";
    const status = linkedListingStatusInfo(listing.status);
    const price = Number(listing.currentPrice);
    const diffTarget = listing.currency === "BRL" && Number.isFinite(price) && price > 0 ? price - Number(state.goal.target || 0) : null;
    return `<div class="dashboard-linked-goal ${status.className}">
      <div><span>${status.icon} Preço vinculado</span><strong>${Number.isFinite(price) && price > 0 ? linkedPrice(price, listing.currency) : "—"}</strong></div>
      <div><span>Última consulta</span><strong>${escapeHtml(formatDateTime(listing.checkedAt))}</strong></div>
      ${diffTarget !== null && Math.abs(diffTarget) >= .005 ? `<small>${diffTarget < 0 ? "O anúncio está " + money(Math.abs(diffTarget)) + " abaixo" : "O anúncio está " + money(diffTarget) + " acima"} do valor-alvo do ciclo.</small>` : ""}
    </div>`;
  }

  function linkedGoalCardHtml() {
    const listing = normalizeLinkedListing(state.goal?.linkedListing);
    if (!listing) {
      return `<section class="linked-goal-card empty">
        <div>
          <span class="linked-goal-eyebrow">🔗 META VINCULADA</span>
          <h3>Acompanhe o preço de um anúncio</h3>
          <p>Cole o link de um produto, serviço ou imóvel. O Nervi só consulta novamente quando você clicar em atualizar.</p>
        </div>
        <button id="linkGoalBtn" class="button secondary" type="button">Vincular anúncio</button>
      </section>`;
    }
    const status = linkedListingStatusInfo(listing.status);
    const price = Number(listing.currentPrice);
    const delta = linkedListingDelta(listing);
    const targetDiff = listing.currency === "BRL" && Number.isFinite(price) && price > 0 ? price - Number(state.goal.target || 0) : null;
    const historyCount = listing.history?.length || 0;
    return `<section class="linked-goal-card ${status.className}">
      <div class="linked-goal-topline">
        <span class="linked-goal-eyebrow">${status.icon} ${escapeHtml(status.label)}</span>
        <span>${escapeHtml(goalPriceTypeLabel(listing.priceType))}</span>
      </div>
      <div class="linked-goal-main">
        ${listing.image ? `<img class="linked-goal-image" src="${escapeHtml(listing.image)}" alt="" referrerpolicy="no-referrer" loading="lazy" onerror="this.style.display='none'">` : ""}
        <div class="linked-goal-copy">
          <h3>${escapeHtml(listing.title)}</h3>
          <div class="linked-goal-price">${Number.isFinite(price) && price > 0 ? linkedPrice(price, listing.currency) : "Preço não identificado"}</div>
          ${delta !== null && Math.abs(delta) >= .005 ? `<p class="${delta < 0 ? "difference-good" : "difference-bad"}">${delta < 0 ? "↓" : "↑"} ${linkedPrice(Math.abs(delta), listing.currency)} desde que o anúncio foi vinculado.</p>` : ""}
          ${targetDiff !== null && Math.abs(targetDiff) >= .005 ? `<p class="muted">${targetDiff < 0 ? `Hoje custa ${money(Math.abs(targetDiff))} menos que o valor-alvo do ciclo.` : `Hoje custa ${money(targetDiff)} mais que o valor-alvo do ciclo.`}</p>` : ""}
          ${linkedListingExtrasHtml(listing)}
          <p class="linked-goal-check">Última consulta: <strong>${escapeHtml(formatDateTime(listing.checkedAt))}</strong>${listing.lastSuccessAt && listing.lastSuccessAt !== listing.checkedAt ? ` · última leitura válida: ${escapeHtml(formatDateTime(listing.lastSuccessAt))}` : ""}</p>
          ${listing.lastError ? `<p class="linked-goal-error">${escapeHtml(listing.lastError)}</p>` : ""}
        </div>
      </div>
      <div class="linked-goal-actions">
        <button id="refreshGoalLinkBtn" class="button primary" type="button">↻ Atualizar preço</button>
        <button id="openGoalLinkBtn" class="button secondary" type="button">Abrir anúncio</button>
        <button id="goalPriceHistoryBtn" class="button secondary" type="button" ${historyCount ? "" : "disabled"}>Histórico (${historyCount})</button>
        <button id="changeGoalLinkBtn" class="button secondary" type="button">Trocar link</button>
        <button id="unlinkGoalLinkBtn" class="button danger" type="button">Desvincular</button>
      </div>
    </section>`;
  }

  function openLinkGoalDialog() {
    const current = normalizeLinkedListing(state.goal?.linkedListing);
    openDynamicDialog({
      title: current ? "TROCAR LINK DA META" : "VINCULAR ANÚNCIO À META",
      body: `
        <div class="dialog-note">A leitura é feita somente agora e quando você clicar em “Atualizar preço”. O Nervi não consulta o anúncio automaticamente.</div>
        <div class="dialog-form-grid">
          <label>Link do anúncio<input id="goalLinkDialogUrl" type="url" inputmode="url" maxlength="1200" placeholder="https://..." value="${escapeHtml(current?.url || "")}"></label>
        </div>
        <div id="dynamicFormError" class="message error hidden"></div>
        <div class="dialog-actions">
          <button id="cancelDynamicForm" class="button secondary" type="button">Cancelar</button>
          <button id="goalLinkDialogSubmit" class="button primary" type="submit">Ler e vincular</button>
        </div>`,
      onOpen: () => setTimeout(() => $("#goalLinkDialogUrl")?.focus(), 0),
      onSubmit: async () => {
        const button = $("#goalLinkDialogSubmit");
        try {
          const url = normalizeGoalLinkUrl($("#goalLinkDialogUrl")?.value || "");
          if (!url) throw new Error("Informe um link válido começando com http:// ou https://.");
          if (button) { button.disabled = true; button.textContent = "Lendo…"; }
          const payload = await readGoalLink(url);
          const previous = current && normalizeGoalLinkUrl(current.url) === url ? current : null;
          state.goal.linkedListing = buildLinkedListing(url, payload, previous);
          saveState();
          closeDynamicDialog();
          renderGoal();
          const listing = normalizeLinkedListing(state.goal.linkedListing);
          showToast(listing?.status === "available" ? "Anúncio vinculado à meta." : "Link salvo. A leitura não encontrou um preço agora.");
        } catch (error) {
          showDynamicError(error?.message || "Não foi possível ler o anúncio.");
          if (button) { button.disabled = false; button.textContent = "Ler e vincular"; }
        }
      }
    });
  }

  async function refreshLinkedGoal() {
    const listing = normalizeLinkedListing(state.goal?.linkedListing);
    if (!listing) return;
    const lastCheck = new Date(listing.checkedAt || 0).getTime();
    if (Number.isFinite(lastCheck) && Date.now() - lastCheck < 30000) {
      showToast("Este anúncio acabou de ser consultado. Aguarde alguns segundos.", "warning");
      return;
    }
    const button = $("#refreshGoalLinkBtn");
    if (button) { button.disabled = true; button.textContent = "Atualizando…"; }
    try {
      const payload = await readGoalLink(listing.url);
      const previousPrice = Number(listing.currentPrice);
      state.goal.linkedListing = buildLinkedListing(listing.url, payload, listing);
      saveState();
      const updated = normalizeLinkedListing(state.goal.linkedListing);
      const newPrice = Number(updated?.currentPrice);
      renderGoal();
      if (updated?.status === "available" && Number.isFinite(newPrice)) {
        if (Number.isFinite(previousPrice) && Math.abs(newPrice - previousPrice) < .005) showToast(`Preço continua ${linkedPrice(newPrice, updated.currency)}.`);
        else if (Number.isFinite(previousPrice)) showToast(`Preço atualizado: ${linkedPrice(newPrice, updated.currency)}.`);
        else showToast(`Preço identificado: ${linkedPrice(newPrice, updated.currency)}.`);
      } else {
        showToast(linkedListingStatusInfo(updated?.status).label, "warning");
      }
    } catch (error) {
      state.goal.linkedListing = normalizeLinkedListing({
        ...listing,
        status: "unreachable",
        checkedAt: nowISO(),
        lastError: error?.message || "Não foi possível consultar o anúncio agora."
      });
      saveState();
      renderGoal();
      showToast("Não foi possível atualizar o anúncio.", "error");
    }
  }

  function openGoalPriceHistory() {
    const listing = normalizeLinkedListing(state.goal?.linkedListing);
    const history = listing?.history || [];
    openDynamicDialog({
      title: "HISTÓRICO DO PREÇO",
      body: `${history.length ? `<div class="table-wrap"><table class="retro-table"><thead><tr><th>Quando</th><th>Preço</th><th>Alteração</th></tr></thead><tbody>${history.map((entry, index) => {
        const previous = index ? Number(history[index - 1].price) : null;
        const diff = previous === null ? null : Number(entry.price) - previous;
        return `<tr><td>${escapeHtml(formatDateTime(entry.checkedAt))}</td><td>${linkedPrice(entry.price, entry.currency || listing.currency)}</td><td>${diff === null || Math.abs(diff) < .005 ? "Valor inicial" : `<span class="${diff < 0 ? "difference-good" : "difference-bad"}">${diff < 0 ? "↓" : "↑"} ${linkedPrice(Math.abs(diff), entry.currency || listing.currency)}</span>`}</td></tr>`;
      }).join("")}</tbody></table></div>` : `<div class="empty-state">Ainda não houve mudança de preço registrada.</div>`}
        <div class="dialog-actions"><button id="cancelDynamicForm" class="button primary" type="button">Fechar</button></div>`,
      onSubmit: () => closeDynamicDialog()
    });
  }

  function bindLinkedGoalEvents() {
    $("#linkGoalBtn")?.addEventListener("click", openLinkGoalDialog);
    $("#changeGoalLinkBtn")?.addEventListener("click", openLinkGoalDialog);
    $("#refreshGoalLinkBtn")?.addEventListener("click", refreshLinkedGoal);
    $("#openGoalLinkBtn")?.addEventListener("click", () => {
      const listing = normalizeLinkedListing(state.goal?.linkedListing);
      if (listing?.resolvedUrl || listing?.url) window.open(listing.resolvedUrl || listing.url, "_blank", "noopener,noreferrer");
    });
    $("#goalPriceHistoryBtn")?.addEventListener("click", openGoalPriceHistory);
    $("#unlinkGoalLinkBtn")?.addEventListener("click", async () => {
      const ok = await confirmAction("Desvincular este anúncio da meta? O histórico de preços vinculado também será removido.");
      if (!ok) return;
      state.goal.linkedListing = null;
      saveState();
      renderGoal();
  ...
  function renderSettings() {
    $("#content").innerHTML = contentWindow("CONFIGURAÇÕES", `
      ...
      <div class="appearance-box security-phone-box">
        <h3>🔐 Acesso e senha</h3>
        <p class="muted">Se você esquecer sua senha ou bloquear a conta após 3 tentativas incorretas, entre em contato com o gerente da Nervi. A redefinição e o desbloqueio são manuais.</p>
        <p class="muted">A opção “Lembrar de mim” mantém apenas a sessão autenticada no dispositivo; o Nervi não salva sua senha em texto no navegador.</p>
      </div>
  ...
  async function submitSuggestion(message) {
    const clean = String(message || "").trim();
    if (!clean) throw new Error("Escreva sua sugestão antes de enviar.");
    if (clean.length > 140) throw new Error("A sugestão deve ter no máximo 140 caracteres.");
    if (!cloud || !activeCloudUserId) throw new Error("Sua sessão não está disponível. Entre novamente.");

    const { error } = await cloud.from("nervi_suggestions").insert({
      user_id: activeCloudUserId,
      username: state.profile?.username || "Usuário",
      message: clean
    });
    if (error) throw error;
  }
  ...
  function openSuggestionDialog() {
    ...
    openDynamicDialog({
      title: "SUGESTÕES",
      body: `
        <p class="dialog-note">Tem alguma ideia para o Nervi? Escreva em até 140 caracteres. O Gabriel poderá visualizar sua mensagem pelo Supabase.</p>
        <label class="suggestion-field">Sua sugestão
          <textarea id="suggestionText" maxlength="140" rows="4" placeholder="Ex.: Seria legal comparar meus gastos com o mês anterior."></textarea>
        </label>
        <div class="suggestion-meta"><span>Obrigado por ajudar a melhorar o projeto.</span><strong id="suggestionCounter">0 / 140</strong></div>
        <div id="dynamicFormError" class="message error hidden"></div>
        <div class="dialog-actions"><button id="cancelDynamicForm" class="button secondary" type="button">Cancelar</button><button id="sendSuggestionBtn" class="button primary" type="submit">Enviar sugestão</button></div>`,
      onOpen: () => {
        const text = $("#suggestionText");
        const counter = $("#suggestionCounter");
        const update = () => { if (counter && text) counter.textContent = `${text.value.length} / 140`; };
        text?.addEventListener("input", update);
        update();
        setTimeout(() => text?.focus(), 0);
      },
      onSubmit: async () => {
        const button = $("#sendSuggestionBtn");
        try {
          if (button) { button.disabled = true; button.textContent = "Enviando…"; }
          await submitSuggestion($("#suggestionText")?.value || "");
          closeDynamicDialog();
          showToast("Sugestão enviada com sucesso. Obrigado!", "success");
        } catch (error) {
          showDynamicError(error?.message || "Não foi possível enviar sua sugestão agora.");
          if (button) { button.disabled = false; button.textContent = "Enviar sugestão"; }
        }
      }
    });
  }
    ...
    openDynamicDialog({
      ...
      body: `
        <div class="help-simple">
          <div><strong>1. Conheça o projeto e solicite acesso</strong><span>Leia a apresentação, escolha um usuário e crie uma senha de exatamente 6 números, sem sequências como 123456. A conta só é liberada pelo gerente.</span></div>
          ...
          <div><strong>7. Envie sugestões</strong><span>No menu de três pontos, use Sugestões para enviar uma ideia de até 140 caracteres diretamente ao gerente.</span></div>
        </div>
  ...
  function showToast(message, kind = "success") {
    ...
    toast.textContent = `${icon} ${cleanDialogText(message)}`;
    toast.classList.remove("success", "error", "warning");
    toast.classList.add(kind === "error" ? "error" : kind === "warning" ? "warning" : "success");
  ...
  function confirmAction(text) {
    return new Promise(resolve => {
      ...
      $("#confirmText").textContent = cleanDialogText(text);
  ...
  function openDynamicDialog({ title, body, onSubmit, onOpen = null }) {
    ...
    $("#formDialogTitle").textContent = cleanDialogText(title);
  ...
  function bindStaticEvents() {
    ...
    $("#aboutBackToLogin")?.addEventListener("click", () => showLogin(true));
    $("#aboutRequestAccess")?.addEventListener("click", beginAccountRequest);
    $("#backToLogin")?.addEventListener("click", showAbout);
    $("#finishWelcome")?.addEventListener("click", () => {
      if (!state.profile) return showMain();
      state.profile.onboardingSeen = true;
      saveState("Boas-vindas concluídas.");
      showMain();
    });
    ...
    $$('input[name="setupGoalMode"]').forEach(input => input.addEventListener("change", () => {
      renderSetupGoalMode();
      setupError();
    }));
    $("#setupReadGoalLink")?.addEventListener("click", handleSetupReadGoalLink);
    $("#setupGoalLink")?.addEventListener("input", event => {
      const currentUrl = normalizeGoalLinkUrl(event.target.value);
      if (setupLinkedGoal && normalizeGoalLinkUrl(setupLinkedGoal.url) !== currentUrl) {
        setupLinkedGoal = null;
        renderSetupGoalLinkPreview();
        renderSetupGoalMode();
      }
    ...
    $("#setupPassword")?.addEventListener("input", updateSetupPinMatchStatus);
    $("#setupPasswordConfirm")?.addEventListener("input", updateSetupPinMatchStatus);
    ...
    $("#quickExpenseFab")?.addEventListener("click", openQuickExpenseDialog);
    document.addEventListener("keydown", event => {
      const target = event.target;
      const typing = target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement || target instanceof HTMLSelectElement || target?.isContentEditable;
      if (typing) return;
      if ((event.ctrlKey || event.metaKey) && event.shiftKey && event.key.toLowerCase() === "g") {
        const anotherDialogOpen = Boolean($("#formDialog")?.open || $("#confirmDialog")?.open);
        if (!anotherDialogOpen && !$("#mainScreen")?.classList.contains("hidden")) {
          event.preventDefault();
    ...
    document.addEventListener("click", event => {
      ...
      const suggestionButton = event.target.closest(".suggestion-action");
      if (suggestionButton) {
        event.preventDefault();
  ...
  async function bootNervi() {
    applyTheme(getTheme(), false);
    if (!cloud) {
      showLogin(true);
      setLoginError("A Nervi Cloud não carregou. Atualize a página.");
      return;
    }
    ...
    try {
      ...
      const username = String(data.session.user.user_metadata?.username || state.profile?.username || "").trim();
      if (!username) {
      ...
      const access = await cloudUsernameStatus(username);
      if (!access.exists || access.status !== "approved" || access.loginLocked) {
        await cloud.auth.signOut({ scope: "local" }).catch(() => undefined);
        showLogin(false);
        if ($("#loginUsername")) $("#loginUsername").value = username;
        if (access.loginLocked) setLoginError("Conta bloqueada após 3 tentativas incorretas. Entre em contato com o gerente da Nervi.");
        else if (access.status === "blocked") setLoginError("O acesso desta conta está bloqueado pelo gerente da Nervi.");
        else if (access.status === "pending") setLoginError("Sua conta ainda aguarda aprovação do gerente da Nervi.", "pending");
        return;
      }

      activeCloudUserId = data.session.user.id;
      await loadCloudState(activeCloudUserId);
      showAfterLogin();
    } catch (error) {
      console.warn("Nervi session restore:", error);
      activeCloudUserId = null;
      showLogin(true);
    }
  }
